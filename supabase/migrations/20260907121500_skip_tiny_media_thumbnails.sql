-- Files below 64 KiB are already cheaper to serve than a useful generated
-- thumbnail. Some legacy objects are also invalid/truncated images, so exclude
-- this tiny set from the actionable backfill count.
  ...
  select jsonb_build_object(
    ...
    'missing_thumbnail_count', count(*) filter (
      where name not like '%-thumb.webp'
        and coalesce((metadata ->> 'size')::bigint, 0) > 65536
...
begin
  ...
  where source.bucket_id = 'project-media'
    ...
    and coalesce((source.metadata ->> 'size')::bigint, 0) > 65536
