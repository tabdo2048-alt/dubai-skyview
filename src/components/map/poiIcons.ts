...
// Raw inner markup (viewBox 0 0 24 24) for each named glyph, WITHOUT the <svg>
// wrapper — so the marker rasteriser (landmarksLayer.ts) can drop them inside a
// composite chip <svg>, and iconFor() can still wrap them for any DOM use. Keep
// each recognisable at ~20px.
...
type PoiLike = { name: string; category: PoiCategory; icon?: string | null };

/** Resolve the glyph KEY (into GLYPH_PATHS) for a landmark, with a per-category
 *  default and a final safe fallback. Used to build the symbol-layer sprite id. */
export function iconKeyFor(poi: PoiLike): string {
  const key = poi.icon || BY_NAME[poi.name] || CATEGORY_DEFAULT[poi.category];
  return GLYPH_PATHS[key] ? key : (GLYPH_PATHS[CATEGORY_DEFAULT[poi.category]] ? CATEGORY_DEFAULT[poi.category] : "landmark");
}

/** Resolve the line-art glyph as a full <svg> string (DOM use). */
export function iconFor(poi: PoiLike): string {
  return svg(GLYPH_PATHS[iconKeyFor(poi)]);
}
